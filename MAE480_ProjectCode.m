...
% fplot(@(x1_cre) -0.0442*x1_cre.^3 + 0.2946*x1_cre.^2 - 0.6771*x1_cre + 0.6100)
...
Area_Main = 576.9%-112.12;    %in2
Area_Horz = 204;       %in2
% Area_Vert = 115.289;   %in2
...
%ta = sqrt(1-M^2); Function to solve for aw
ao = @(ao_theory,ao_O_ao_theory,M) (1.05/sqrt(1-M^2))*(ao_O_ao_theory)*(ao_theory);
k = @(ao) ao/(2*pi);
...
R_LS_Wing = R_LS((deg2rad(Main_Lambda)));

Swet_o_S = (2*Area_Main)/Area_Main;

C_Do_w = C_fw*(1+L+100*0.124^4)*R_LS_Wing*Swet_o_S;

dq_o_q = 2.42*(sqrt(C_Do_w))/((lh/cbarw)+0.3);

neta = 1-dq_o_q;

lift_curve_slope = aW_Main+aW_tail*(1-de_o_da_tail)*neta*(Area_Horz/Area_Main);
...
dcm_o_da = aW_Main*x_bar_a+Cmaf-aW_tail*(1-de_o_da_tail)*neta*(Area_Horz/Area_Main);

N_O =  x_bar_ac_wb + (Cmaf/aW_Main) - (aW_tail/aW_Main)*(1-de_o_da_tail)*bar_v1*neta
